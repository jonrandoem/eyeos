...
/**
 * A list is unassigned from a single or mutiple contact
 * 
...
class UnassignListEvent implements ISimpleEventHandler {
	/**
	...
	public function autoFill (AbstractEventNotification $event) {
		...
		if (!is_array($userIds)) {
			...
			$event->setMessageInformation(array('User %s was deleted from %s list.', array($username, $listName)));
		} else {
			$usernames = array();
			...
			$event->setMessageInformation(array('Users %s were deleted from %s list.', array($usernamesString, $listName)));
		}
