 ...
 * @subpackage Security
 */
class SystemGroupSecurityHandler implements ISecurityHandler {
	...
	public function __construct(array $params = null) {
		if (!isset($params[self::PARAM_GROUPS])) {
			//this is currently the only parameter needed, so it is mandatory
			throw new EyeMissingArgumentException('Missing parameter $params[' . self::PARAM_GROUPS . ']');
		}
		if (is_string($params[self::PARAM_GROUPS])) {
			$this->groups = array_map('trim', explode(SecurityConstants::PARAMS_VALUES_DELIMITER, $params[self::PARAM_GROUPS]));
		} else {
			throw new EyeInvalidArgumentException('Parameter $params[' . self::PARAM_GROUPS . '] must be a string.');
		}
	}
