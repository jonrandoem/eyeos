...
abstract class SessionApplication extends EyeosApplicationExecutable {
        private static $initApps = array('desktop', 'contactmanager', 'netsync');
	...
	public static function __run(AppExecutionContext $context, MMapResponse $response) {
		...
		foreach($procList as $proc) {
			if ($proc->getName() == 'login') {
				try {
					$PM->kill($proc);
				} catch (Exception $e) {
					Logger::getLogger('eyeos.session')->warn('Cannot kill login process ' . $proc . ': ' . $e->getMessage());
				}
			}
		}
