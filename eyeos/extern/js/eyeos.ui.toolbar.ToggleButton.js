...
/**
 *	eyeos.ui.toolbar.ToggleButton - Styling...
 *	Extending a qx.ui.toolbar.RadioButton, to implement the eyeos
 *	look and feel behaviour.
 */
qx.Class.define('eyeos.ui.toolbar.ToggleButton', {
	extend : qx.ui.toolbar.RadioButton,

	construct : function(item, iconsPath, actions) {
		arguments.callee.base.call(this, tr(item.name), iconsPath + item.image);
		this.setId(item.id);
	...
	members: {
		...
		_setEyeosStyle: function(item, actions) {
			...
			if (item.cmd) {
				...
				this.addListener('click', new Function('foo', 'this.actions.' + item.cmd + '(foo);'));
			}
